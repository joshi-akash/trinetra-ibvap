...
def _to_entity_out(e: EntityLog) -> EntityLogOut:
    return EntityLogOut(
        ...
        plate_thumbnail_path=getattr(e, "plate_thumbnail_path", None),
...
def list_entities(
    ...
    if plate:
        clean_search = plate.replace(" ", "").replace("-", "").upper()
        query = query.filter(EntityLog.plate_text.ilike(f"%{clean_search}%"))
    if has_plate is True:
        query = query.filter(EntityLog.plate_text.isnot(None), EntityLog.plate_text != "")
    elif has_plate is False:
        query = query.filter(or_(EntityLog.plate_text.is_(None), EntityLog.plate_text == ""))
    if color:
        query = query.filter(
            or_(
