...
def detect_camera_frame(
    ...
    # Environmental Weather & Visibility Evaluation
    try:
        from ai_behavior.environmental import compute_weather_and_visibility
        env_state = compute_weather_and_visibility(frame)
        is_low_light = bool(env_state.is_low_light)
        weather_condition = env_state.weather_condition
        visibility_score = env_state.visibility_score
        enhanced_mode = env_state.recommended_preprocessing
        active_model_name = f"yolov8_{env_state.recommended_model}.pt"
    except Exception:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) if (len(frame.shape) == 3 and frame.shape[2] == 3) else frame
        mean_lum = float(np.mean(gray))
        is_low_light = bool(mean_lum < 85.0)
        weather_condition = "NIGHT_LOW_LIGHT" if is_low_light else "CLEAR_DAY"
        visibility_score = 0.45 if is_low_light else 0.90
        enhanced_mode = "zero_dce_night_vision" if is_low_light else "standard"
        active_model_name = "yolov8_night.pt" if is_low_light else "yolov8_custom.pt"
    ...
    try:
        ...
        raw_entities = run_detection_stage(frame, camera_id=camera_id, calibration_data=calib_data, is_low_light=is_low_light)
    except Exception as e:
    ...
    for ent in raw_entities:
        ...
        if geofence_coords and len(geofence_coords) >= 3:
            is_breach = geofence_evaluator.bbox_intersects_geofence(bbox, None, geofence_coords) or \
                        geofence_evaluator.is_inside_geofence(ent_lat, ent_lon, geofence_coords)
        ...
        # Ensure active threat alerts require adequate detection confidence (conf >= 0.30)
        # Prevents false positive alarms on low-confidence ghost detections
        if is_alert and conf_val < 0.30:
            is_alert = False
            alert_type = None
            rule_fired = None
            threat_score = 0.10
        ...
        if should_log:
            ...
            # For all logged entities (alerts AND passive vehicles/humans), save high-quality real crop & scene snapshot
            thumb_name = f"thumb_{ent_id[:8]}.webp"
            snap_name = f"snap_{ent_id[:8]}.jpg"
            thumb_file = thumb_dir / thumb_name
            snap_file = thumb_dir / snap_name
            ...
            try:
                ...
                if crop.size > 0:
                    cv2.imwrite(str(thumb_file), crop, [cv2.IMWRITE_WEBP_QUALITY, 85])
                    thumb_rel = f"/storage/thumbnails/{thumb_name}"

                # Save full scene snapshot with tactical target bounding box
                snap_img = frame.copy()
                box_color = (34, 197, 94) if attrs.get("plate_text") else ((34, 34, 239) if is_alert else (56, 189, 248))
                cv2.rectangle(snap_img, (max(0, x1), max(0, y1)), (min(w, x2), min(h, y2)), box_color, 2)
                tag_label = f"{ent_type.upper()}"
                if attrs.get("plate_text"):
                    tag_label += f" [{attrs['plate_text']}]"
                elif attrs.get("face_name") and attrs.get("face_name") != "Unidentified":
                    tag_label += f" [{attrs['face_name']}]"
                cv2.putText(snap_img, tag_label, (max(0, x1), max(18, y1 - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55, box_color, 2)
                cv2.imwrite(str(snap_file), snap_img, [cv2.IMWRITE_JPEG_QUALITY, 90])
            except Exception as e:
                logger.warning(f"Error saving entity thumbnail & snapshot: {e}")

            # Save dedicated plate crop screenshot for vehicles (linking to vehicle details)
            plate_rel = None
            if ent_type == "vehicle":
                p_name = f"plate_{ent_id[:8]}.webp"
                p_file = thumb_dir / p_name
                plate_crop = attrs.get("plate_crop")
                if plate_crop is not None and getattr(plate_crop, "size", 0) > 0:
                    try:
                        cv2.imwrite(str(p_file), plate_crop, [cv2.IMWRITE_WEBP_QUALITY, 90])
                        plate_rel = f"/storage/thumbnails/{p_name}"
                    except Exception as e:
                        logger.warning(f"Error saving plate crop: {e}")
                elif crop is not None and getattr(crop, "size", 0) > 0:
                    try:
                        vh, vw = crop.shape[:2]
                        p_fallback = crop[int(vh * 0.50):vh, int(vw * 0.12):int(vw * 0.88)]
                        if p_fallback.size > 0:
                            cv2.imwrite(str(p_file), p_fallback, [cv2.IMWRITE_WEBP_QUALITY, 90])
                            plate_rel = f"/storage/thumbnails/{p_name}"
                    except Exception:
                        pass
                attrs["plate_thumbnail_path"] = plate_rel

            # Push live frame into ring buffer for evidence recording
            try:
                from backend.app.ingestion.rtsp_service import rtsp_service
                _, frame_enc = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 75])
                rtsp_service.push_frame(
                    camera_id=camera_id,
                    frame_id=ent_id,
                    timestamp=datetime.now(timezone.utc),
                    image_bytes=frame_enc.tobytes()
                )
            except Exception:
                pass

            # For active threat alerts (suspicious movement, loitering, sprinting, weapons, breach), extract intervention clip
            clip_path = None
            if is_alert:
                try:
                    from backend.app.ingestion.rtsp_service import rtsp_service
                    extracted_clip = rtsp_service.extract_30s_clip(
                        camera_id=camera_id,
                        trigger_time=datetime.now(timezone.utc),
                        alert_id=ent_id
                    )
                    if extracted_clip and Path(extracted_clip).exists():
                        clip_path = f"/storage/clips/{ent_id}.mp4"
                except Exception as e:
                    logger.warning(f"Error extracting alert intervention clip: {e}")
            ...
            log_entry = EntityLog(
                ...
                plate_thumbnail_path=plate_rel,
            ...
            # Real-Time Threat Alert Dispatch to Admin & Connected Consoles
            if is_alert:
                alert_payload = {
                    ...
                    "thumbnail_url": thumb_rel or f"/storage/thumbnails/snap_{ent_id[:8]}.jpg",
                    "keyframe_webp_url": thumb_rel,
                    "clip_url": clip_path,
                    "clip_mp4_url": clip_path,
                    "entity": {
                        "entity_type": ent_type,
                        "upper_color": attrs.get("upper_color"),
                        "lower_color": attrs.get("lower_color"),
                        ...
                        "speed_kmh": attrs.get("speed_kmh"),
                        "direction": attrs.get("direction"),
                        "location": {"lat": ent_lat, "lon": ent_lon}
                    }
                }
                try:
                    ...
                    logger.warning(f"Error broadcasting alert over websocket: {e}")

                try:
                    from backend.app.alert_manager.package_builder import alert_manager
                    alert_manager.publish_mqtt_event("trinetra/alerts", alert_payload)
                except Exception:
                    pass
