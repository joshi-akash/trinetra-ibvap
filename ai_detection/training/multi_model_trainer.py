...
class MultiModelTrainer:
    ...
    def _init_state(self):
        ...
        self.models: List[ModelProgress] = [
            ...
            ModelProgress(
                id="visdrone_overhead",
                name="VisDrone Overhead & High-CCTV",
                dataset_yaml="data/visdrone/data.yaml",
                target_weights="models/yolov8_visdrone.pt",
                total_epochs=5,
            ),
        ]
    ...
    def start_single_model_training(self, model_id: str, epochs: Optional[int] = None) -> bool:
        with self._lock:
            if self.is_training:
            ...
            self.active_model_index = target_idx
            self.start_time = time.time()
            self.end_time = 0.0
            self.logs.clear()

            target_m.status = "PENDING"
            target_m.progress = 0.0
            target_m.current_epoch = 0
            target_m.box_loss = 0.0
            target_m.cls_loss = 0.0
            target_m.error = None
            if epochs and epochs > 0:
                target_m.total_epochs = epochs

            self._thread = threading.Thread(
                target=self._run_single_pipeline,
                args=(target_m, epochs),
                daemon=True,
            )
            self._thread.start()
            self.log(f"Single model training launched for {target_m.name} ({target_m.id}).")
            return True

    def _train_single_model(self, m: ModelProgress, epochs: Optional[int] = None):
        ...
        try:
            ...
            num_epochs = epochs if (epochs and epochs > 0) else m.total_epochs
    ...
    def _run_single_pipeline(self, m: ModelProgress, epochs: Optional[int] = None):
        try:
            self._train_single_model(m, epochs=epochs)
            try:
                from ai_detection import reload_detector
                reload_detector(m.target_weights)
                self.log(f"Active detector reloaded with refined weights: {m.target_weights}")
            except Exception as e:
                self.log(f"Reload note: {e}")
            ...
            self.log(f"Dedicated retraining complete for {m.name}!")
        except Exception as ex:
            logger.error(f"Single model training failed: {ex}")
            with self._lock:
                self.is_training = False
                self.all_completed = False
                self.end_time = time.time()
            self.log(f"Fatal error in single model training: {ex}")
    ...
    def start_batch_training(self) -> bool:
        with self._lock:
            ...
            self._thread.start()
            self.log("Batch training pipeline launched for all 5 surveillance models.")
            return True
