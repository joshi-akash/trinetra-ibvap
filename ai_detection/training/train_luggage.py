...
def convert_voc_to_yolo():
    logger.info("Starting VOC to YOLO conversion for Luggage dataset...")
    
    # User's extracted paths
    base_dir = Path(r"E:\project\ai_detection\data\luggage")
    ann_dir = base_dir / "annotation" / "annotation"
    img_dir = base_dir / "suitcase" / "suitcase"
    ...
    classes = {"suitcase": 0, "luggage": 0, "bag": 0}  # Map all to class 0
    ...
    for xml_file in xml_files:
        ...
        with open(txt_path, 'w') as out_file:
            for obj in root.iter('object'):
                ...
                out_file.write(f"0 {b_center_x:.6f} {b_center_y:.6f} {b_width:.6f} {b_height:.6f}\n")
                has_boxes = True
    ...
    # Generate YAML
    yaml_path = yolo_base / "luggage.yaml"
    with open(yaml_path, 'w') as f:
        f.write(f"path: {yolo_base.absolute().as_posix()}\n")
        f.write("train: images\n")
        f.write("val: images\n")
        f.write("nc: 1\n")
        f.write("names: ['abandoned_luggage']\n")
        
    return yaml_path

def train_luggage_model():
    ...
    logger.info("Starting intensive training on Abandoned Luggage...")
    model.train(
        data=str(yaml_path),
        epochs=10,
        imgsz=320,  # Lowered from 640 to prevent OOM
        batch=4,    # Lowered from 16 to prevent OOM
        workers=0,
        name="luggage_detector_run",
        project=r"E:\backend_cctv\runs\train",
        exist_ok=True
    )
    
    best_weights = Path(r"E:\backend_cctv\runs\train\luggage_detector_run\weights\best.pt")
    target_path = Path(r"E:\backend_cctv\models\luggage_yolov8n.pt")
    
    if best_weights.exists():
        target_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy(str(best_weights), str(target_path))
        logger.info(f"Successfully saved Abandoned Luggage Detector to: {target_path}")
